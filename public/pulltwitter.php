...
class TwitterSentimentAnalysis
{
    ...
    public function sentimentAnalysisNewsArticle($newsarticle)
    {

        return $this->findSentimentNewsArticles($newsarticle);
    }
    ...
    protected function findSentiment($tweets)
    {
        ...
        foreach ($tweets['statuses'] as $tweet) { //foreach of the tweets that we received
            if (isset($tweet['metadata']['iso_language_code']) && $tweet['metadata']['iso_language_code'] == 'en') { //perform sentiment analysis only for the English Tweets
                ...
                $sentimentx = new \PHPInsight\Sentiment();
                //foreach ($strings as $string) {

                    // calculations:
                    //$sentiment = $sentimentx->score($tweet['text']);
                $sentiment = $sentimentx->categorise($tweet['text']);
    ...
    protected function findSentimentNewsArticles($newsarticle)
    {
        $sentimentx = new \PHPInsight\Sentiment();
        $sentiment = $sentimentx->categorise($newsarticle);

        //$DatumboxAPI = new DatumboxAPI($this->datumbox_api_key); //initialize the DatumboxAPI client

        //$sentiment = $DatumboxAPI->TwitterSentimentAnalysis($newsarticle); //call Datumbox service to get the sentiment
